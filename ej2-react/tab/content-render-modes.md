...
In the following code example, the Calendar and Scheduler are rendered in the first and second tabs, respectively. Initially, the Scheduler is not available, but it will be rendered once the second tab is selected. Both the Calendar and Scheduler are then maintained in the DOM.
...
In the following example, all three tabs are rendered on the initial load, and the data entered in the first tab will be maintained, even when the second or third tab is active. In Init mode, every child component is rendered into the DOM, so you can access component references for any tab using the `ref` attribute.
