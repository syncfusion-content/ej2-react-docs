...
**Through interaction:** Click the clear icon in the React Dropdown List element to clear the selected item. Enable the clear icon using the [`showClearButton`](https://ej2.syncfusion.com/react/documentation/api/drop-down-list#showclearbutton) property.

**Programmatically:** Set the `index`, `text`, or `value` property to `null` to clear the selected item.

The following example demonstrates how to clear the selected item in React Dropdown List.
