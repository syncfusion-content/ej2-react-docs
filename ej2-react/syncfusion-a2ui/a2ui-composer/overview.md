...
description: Learn what the Syncfusion A2UI Composer Playground is, who it is for, and how it fits into the A2UI for React workflow.
...
The Syncfusion<sup style="font-size:70%">&reg;</sup> A2UI Composer Playground is an AI-powered, browser-based designer for authoring [A2UI v0.9](https://a2ui.org/specification/v0.9-a2ui/) JSON, the declarative wire format that the [Syncfusion A2UI for React package](https://www.npmjs.com/package/@syncfusion/ej2-react-a2ui) renderer turns into fully interactive Syncfusion EJ2 React surfaces.

Open the Composer, describe the UI you want, refine the output, and copy the JSON straight into your React application or your Syncfusion A2UI Agent. No component code is written by hand.
...
- **Preview live.** The right-hand preview renders the surface with the real Syncfusion components, exactly as `@syncfusion/ej2-react-a2ui` will render it at runtime.
...
The Composer is the design side of the A2UI for React stack. The runtime side, validation, catalog, and renderer, ships in the [Syncfusion A2UI for React package](https://www.npmjs.com/package/@syncfusion/ej2-react-a2ui); the Composer only authors JSON, no React code is generated or executed.
...
If you are wiring up the runtime side (a React app that already has `<SyncfusionA2UIProvider/>` mounted), see [Getting Started with A2UI for React](../getting-started) and [AI Integration](../ai-integration) instead.
...
- **Designers and product owners** who want to see a Syncfusion-styled surface without writing React.
...
2. You copy the JSON and bind it to an agent (or paste it directly into a React app).
3. The agent echoes that structure verbatim on every request; only data values change.
4. The React app renders the surface with `<SyncfusionA2UIProvider surface={surface}/>`.
