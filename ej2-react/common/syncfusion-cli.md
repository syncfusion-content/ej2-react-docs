...
For React development, the Syncfusion<sup style="font-size:70%">&reg;</sup> CLI supports project creation with **Vite** and **Next.js**. By using the Syncfusion<sup style="font-size:70%">&reg;</sup> CLI, developers can reduce manual configuration effort and quickly start building React projects with a recommended project structure, predefined templates, and the required component setup.
...
- **Component Templates** - Start with ready-to-use React component templates such as `Grid`, `Chart`, `Scheduler`, `Gantt`, `File Manager`, `Diagram`, and `Rich Text Editor`, `PDF Viewer`, `DOCX Editor` and `Spreadsheet Editor`.
...
- **AI Assistant Integration** - Configure MCP assistants and install Syncfusion<sup style="font-size:70%">&reg;</sup> components Skills to improve AI-assisted React development.
...
<td>Creates a new React project using Syncfusion React components.</td>
<td>sf new</td>
</tr>
<tr>
<td>Add component to existing project</td>
<td>Adds a Syncfusion React component to an existing project.</td>
<td>sf add</td>
</tr>
<tr>
...
The following example uses the `new` command to create a React Vite project with Syncfusion<sup style="font-size:70%">&reg;</sup> React components using the Grid template and Material 3 theme.
...
The `new` command creates a React project with Syncfusion<sup style="font-size:70%">&reg;</sup> React components. You can use this command to specify the framework, build tool, project type, component template, theme, styling format, MCP assistant configuration, and component skills installation.

#### Syntax
  
{% tabs %}
{% highlight bash tabtitle="CMD" %}
sf new <project-name> --framework <framework> --build-tool <buildTool> --type <type> --template <component> --theme <theme> --styling <styling> --mcp <editor> --skills
{% endhighlight %}
{% endtabs %}

#### Example
  
The following command creates a React application using Vite with JavaScript, Grid template, Material 3 theme, CSS styling, Syncfusion<sup style="font-size:70%">&reg;</sup> Code Studio MCP configuration, and skills installation.

{% tabs %}
{% highlight bash tabtitle="CMD" %}
sf new my-app --framework react --build-tool vite --template grid --mcp code-studio --skills
{% endhighlight %}
{% endtabs %}
...
<td><code>react</code>, <code>angular</code>, <code>vue</code></td>
</tr>
<tr>
<td><code>--build-tool</code></td>
<td>Specifies the React build tool used to create the application.</td>
<td><code>vite</code>, <code>nextjs</code></td>
</tr>
<tr>
...
> **Note:** For React applications, the `--build-tool` option supports `vite` and `nextjs`.
