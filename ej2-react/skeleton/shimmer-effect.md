...
Shimmer effects provide animated visual feedback during content loading, signaling to users that the application is actively retrieving content. Configure the animation style using the [`shimmerEffect`](https://ej2.syncfusion.com/react/documentation/api/skeleton#shimmereffect) property. The React Skeleton component supports three shimmer effect types: Wave (default), Pulse, and Fade. Each effect creates a different visual impression while conveying the same "loading" message.

## Effect types

The React Skeleton component supports the following shimmer effects:

* **Wave** (default) - Creates a flowing wave animation across the React Skeleton, mimicking scanning motion
...
The following example demonstrates a list with pulse effect React Skeleton:
