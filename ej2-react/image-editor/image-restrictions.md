---
layout: post
title: Image Restrictions in React Image Editor | Syncfusion
description: Restrict uploaded image extensions and enforce minimum and maximum file sizes in the React Image Editor using uploadSettings.
...
# Image Restrictions in React Image Editor
...
*Note:* File restrictions apply when uploading images to the React Image Editor, whether through the open method or the built-in uploader. If `uploadSettings` is not defined, the React Image Editor allows files with default extensions — see the Allowed image extensions section.

## Allowed image extensions

The React Image Editor allows users to specify acceptable file extensions for uploaded images using the [`uploadSettings.allowedExtensions`](https://ej2.syncfusion.com/react/documentation/api/image-editor/uploadSettingsModel#allowedextensions) property, ensuring that only supported formats, such as `.jpg`, `.png`, `.svg`, `.webp`, and `.bmp`, are allowed. This helps maintain compatibility and prevents errors caused by unsupported file types. By default, the React Image Editor allows files with `.jpg, .png, .svg, .webp, and .bmp` extensions.

*Note:* To specify allowed extensions in the upload settings, use the exact string format '.png, .svg', listing the permitted file types as a comma-separated string.
...
*Note:* Users can also upload images as base64 strings; in that case file extension validation is bypassed but file size restrictions still apply.

Example: minimum/maximum size configuration.
