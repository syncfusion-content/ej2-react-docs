---
layout: post
title: React DataManager - ODataV4 Adaptor | Syncfusion
description: React DataManager ODataV4 provides server-side integration with REST APIs by detailing the backend setup and endpoint configuration required for data operations.
...
To create the project, launch **Visual Studio 2022 or later** and select **Create a new project**. Search for the **ASP.NET Core with React.js** template and proceed by clicking Next. Configure the project settings by naming it **ODataV4Adaptor** and choosing a suitable location, then click Create. 
...
When using the `OdataV4Adaptor`, every backend API endpoint must return data in a specific JSON structure. This ensures that Syncfusion<sup style="font-size:70%">&reg;</sup> React DataManager can correctly interpret the response and bind it to the component. The expected format is:
