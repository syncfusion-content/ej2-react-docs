...
Input element values within the React Dialog can be read by binding a click handler to the footer buttons. The [`buttons`](https://ej2.syncfusion.com/react/documentation/api/dialog#buttons) property accepts an array of button configurations, each with a `click` handler. Each button configuration uses a `buttonModel` field that follows the Syncfusion [`ButtonModel`](https://ej2.syncfusion.com/react/documentation/api/button#buttonmodel) interface (for example, `content`, `cssClass`, `isPrimary`). For detailed information, refer to the [footer](../template#footer) section.

The approach is as follows:

1. Use React refs (`ref`) on each input element inside the React Dialog content.
2. In the footer button's `click` handler, read the `.value` property of each ref to retrieve the input values.
3. Display the values in a confirmation React Dialog.
