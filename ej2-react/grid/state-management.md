...
The React Data Grid component supports saving and restoring state using local storage. This feature preserves the current state of the grid, such as column order, sorting, and filtering, enabling a return to previous configurations.
...
When the [enablePersistence](https://ej2.syncfusion.com/react/documentation/api/grid#enablepersistence) property is set to `true` in the React Data Grid component, column configurations are automatically persisted. To add new columns to an already persisted state, the gridâ€™s built-in methods such as `push` can be used, followed by calling the [refreshColumns](https://ej2.syncfusion.com/react/documentation/api/grid#refreshcolumns) method to update the UI with the newly added columns.
