...
To expand the grid detail row using an external button, invoke the [expand](https://ej2.syncfusion.com/react/documentation/api/grid/detailRow#expand) method provided by the `detailRowModule` of the [React Data Grid](https://www.syncfusion.com/react-components/react-data-grid) library. This method expands the detail row of a specific grid row.
