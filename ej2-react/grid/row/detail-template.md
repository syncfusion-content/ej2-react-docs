...
To expand the grid detail row using an external button, invoke the [expand](https://ej2.syncfusion.com/react/documentation/api/grid/detailRow#expand) method provided by the `detailRowModule` of the React DataGrid library. This method expands the detail row of a specific grid row.
...
The detail template icon in the React DataGrid expands or collapses the detail content of a row. By default, the icon represents a right arrow for the collapsed state and a down arrow for the expanded state. To customize this icon, override the following CSS styles:
