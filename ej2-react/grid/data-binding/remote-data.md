...
const data = new DataManager({ url: 'https://services.syncfusion.com/react/production/api/Orders', // Replace with your actual API endpoint URL
adaptor: new UrlAdaptor() });
<GridComponent dataSource={data} allowPaging={true}>...</GridComponent>
```
...
const dataStateChange = (state) => {
  // Fetch data from your service based on the current grid state (page, sort, filter)
  fetchData(state).then((result) => {
    setData({ result: result.data, count: result.total });
  });
};

<GridComponent dataSource={data} dataStateChange={dataStateChange} allowPaging={true}>
  ...
</GridComponent>
```
...
|-----------------------|------------------------------------------------------|-------------------------------------------------------------------------------|
| Django                | [Django guide](../connecting-to-backends/django-server)    | https://github.com/SyncfusionExamples/ej2-react-grid-samples/tree/master/connecting-to-backends/syncfusion-react-grid-custom-binding-with-django-server |
| Express.js            | [Express.js guide](../connecting-to-backends/express-js)   | https://github.com/SyncfusionExamples/ej2-react-grid-samples/tree/master/connecting-to-backends/syncfusion-reactgrid-with-express-server |
| FastAPI               | [FastAPI guide](../connecting-to-backends/fastapi-server)  | https://github.com/SyncfusionExamples/ej2-react-grid-samples/tree/master/connecting-to-backends/syncfusion-reactgrid-with-fastapi-server |
| Flask API             | [Flask API guide](../connecting-to-backends/flaskapi-server) | https://github.com/SyncfusionExamples/ej2-react-grid-samples/tree/master/connecting-to-backends/syncfusion-reactgrid-with-flaskapi-server |
| Next.js               | [Next.js guide](../connecting-to-backends/next-js-server) | https://github.com/SyncfusionExamples/ej2-react-grid-samples/tree/master/connecting-to-backends/syncfusion-reactgrid-with-nextjs-server/Connecting_to_Next_js_server |