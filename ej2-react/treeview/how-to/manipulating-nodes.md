...
The React Tree View component provides Essential<sup style="font-size:70%">&reg;</sup> methods for dynamically managing nodes, offering the ability to create a highly interactive and customizable tree structure,
...
These methods provide the flexibility to add, remove, update, refresh, or relocate nodes as needed, facilitating a fully interactive and customizable React Tree View structure.

## Dynamically adding nodes

The [`addNodes`](https://ej2.syncfusion.com/react/documentation/api/treeview/#addnodes) method of React Tree View allows you to insert new nodes at designated positions within the React Tree View by passing the necessary node information. You can add both parent and child nodes by specifying their target ID.
...
The React Tree View [`removeNodes`](https://ej2.syncfusion.com/react/documentation/api/treeview/#removenodes) method lets you remove multiple nodes by providing their IDs. You can remove both parent and child nodes.
...
The React Tree View component has the [`updateNode`](https://ej2.syncfusion.com/react/documentation/api/treeview/#updatenode) method, which allows you to change a specific node's text by providing its target (either the node ID or element) and the new text. To enable text editing, set the [`allowEditing`](https://ej2.syncfusion.com/react/documentation/api/treeview#allowediting) property to true, ensuring correct functionality of the `updateNode` method.
...
The [`refreshNode`](https://ej2.syncfusion.com/react/documentation/api/treeview/#refreshnode) method in React Tree View allows you to update the content of a specific node by providing its target and the new details. To retrieve the current details of the node, use the [`getTreeData`](https://ej2.syncfusion.com/react/documentation/api/treeview#gettreedata) method in conjunction with the node's ID. This method refreshes a designated node within the React Tree View.
...
The [`moveNodes`](https://ej2.syncfusion.com/react/documentation/api/treeview/#movenodes) method in React Tree View allows you to relocate a node by defining the node to be moved, the target location, and the index within that target. It facilitates the repositioning of nodes within the same React Tree View based on the specified target.
