...
The [`tooltipText`](https://ej2.syncfusion.com/react/documentation/api/toolbar/item#tooltiptext) property of the React Toolbar item (a `string` with a default of `''`) is used to set an HTML Tooltip on the commands that can be viewed as hint text on hover. You can set it per `ItemDirective`.
...
1. Initialize the Tooltip with the React Toolbar target using the `target` option (e.g., `target: '#toolbar'`), which attaches the Tooltip to the command elements.

* Import the `Tooltip` module from `ej2-popups`,and initialize the Tooltip with the React Toolbar target. Refer to the following code example:
