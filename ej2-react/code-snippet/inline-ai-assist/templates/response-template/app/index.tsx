import { InlineAIAssistComponent, InlinePromptRequestEventArgs, PromptResponseModel, ResponseItemSelectEventArgs } from '@syncfusion/ej2-react-interactive-chat';
...
const App: React.FC = () => {
    ...
    const responseItemContent = (ctx: any) => {
        return <div className="responseItemContent">
            <div className="response-header">
                <span className="e-icons e-assistview-icon"></span>
                Inline AI Assist
            </div>
            <div className="responseContent">{ctx.response}</div>
        </div>;
    };

    const handlePromptRequest = (args: InlinePromptRequestEventArgs): void => {
        setTimeout(() => {
            let foundPrompt = promptsData.find((promptObj: PromptResponseModel) => promptObj.prompt === args.prompt);
