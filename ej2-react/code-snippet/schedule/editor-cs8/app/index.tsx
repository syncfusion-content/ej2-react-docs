...
function App() {
  ...
  function onPopupClose(args: PopupCloseEventArgs): void {
    if (args.type === 'Editor' && !isNullOrUndefined(args.data as { [key: string]: Object })) {
      ...
      ((args.data as { [key: string]: Object })).StartTime = startObj.value;
      ((args.data as { [key: string]: Object })).EndTime = endObj.value;
      let descriptionElement: HTMLInputElement = args.element.querySelector('#Description') as HTMLInputElement;
      if (descriptionElement) {
  ...
  function editorTemplate(props: Object): JSX.Element {
    return (props !== undefined ? <table className="custom-event-editor" style={{ width: '100%', padding: '5' }}><tbody>
      ...
      <tr><td className="e-textlabel">From</td><td colSpan={4}>
        <DateTimePickerComponent format='dd/MM/yy hh:mm a' id="StartTime" data-name="StartTime" ref={(date) => { startObj = date }} value={new Date((props as any).startTime || (props as any).StartTime)}></DateTimePickerComponent>
      </td></tr>
      <tr><td className="e-textlabel">To</td><td colSpan={4}>
        <DateTimePickerComponent format='dd/MM/yy hh:mm a' id="EndTime" data-name="EndTime" ref={(date) => { endObj = date }} value={new Date((props as any).endTime || (props as any).EndTime)}></DateTimePickerComponent>
      </td></tr>
