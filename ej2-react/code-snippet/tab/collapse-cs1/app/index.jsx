...
const App = () => {
  ...
  const [trgIndex, setrgIndex] = useState(0);
  let actLine;
  useEffect(() => {
    updateCollapseClass = updateCollapseClass;
  }, []);
  let updateCollapseClass = (index) => {
    // Custom classes are added/removed from tab content and active line element, when the same tab item again clicked
    const cntEle = tabInstance.current.element.querySelector("#e-content-element_" + index);
    actLine = tabInstance.current.element.querySelector('.e-indicator');
  ...
  const tabCreated = () => {
    ...
    actLine = tabInstance.current.element.querySelector('.e-indicator') as any;
    if (tabInstance.current.element.querySelector(".e-content-element_0") != null)
      (tabInstance.current.element.querySelector(".e-content-element_0") as any).classList.add('collapse');
    actLine.classList.add('collapse');
  }

  const tabSelected = (e) => {
    // If next tab item selected custom class is removed from content and active line element
    let cnttrgs = tabInstance.current.element.querySelectorAll('#element.e-tab > .e-content > .e-item');
    for (let i = 0; i < cnttrgs.length; i++) {
      cnttrgs[i].classList.remove('collapse');
    }
    if (actLine !== undefined) { actLine.classList.remove('collapse'); }
    setrgIndex(e.selectedIndex);
  ...
  return (
    <TabComponent heightAdjustMode='Auto' className="e-background" id='element' ref={tabInstance} created={tabCreated} selected={tabSelected}>
      <TabItemsDirective>
