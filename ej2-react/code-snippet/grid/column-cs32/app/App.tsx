...
function App() {
  ...
  const customerTemplate = (props: any): any => {
     return (<div>
          <span className="e-icons e-header" ></span>Customer ID
       </div>);
  }
  const columnTemplate: any = gridTemplate;
  const gridTemplate = (props: any): any => {
    ...
    return( <div> <ButtonComponent onClick= { clickHandler }>Restore</ButtonComponent> <GridComponent id="Grid" dataSource={data} allowFiltering={true} allowPaging={true} enablePersistence={true} height={230}
    ref={g => grid = g}>
        <ColumnsDirective>
