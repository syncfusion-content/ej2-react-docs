...
export function domVirtualizationDataSource() {
    ...
    for (let warehouseIndex = 1; warehouseIndex <= 25000; warehouseIndex++) {
        ...
        const regionIndex = (warehouseIndex - 1) % warehouseRegions.length;
        const childRecords = [];
        ...
        for (let itemIndex = 1; itemIndex <= 3; itemIndex++) {
            ...
            const region = warehouseRegions[regionIndex];
            const cities = countryCities[region];
            const childRegion = cities[(warehouseIndex + itemIndex) % cities.length];
            const childItemName = childRegion + ' Facility';
            const categoryIndex = (warehouseIndex + itemIndex) % categories.length;
            const supplierIndex = (warehouseIndex + itemIndex) % suppliers.length;
            const monthlyUnitsSold = 200 + ((warehouseIndex * (itemIndex + 9)) % 2500);
            const unitPrice = 15 + ((warehouseIndex * itemIndex * 13) % 250);
            let quantity = 100 + ((warehouseIndex * itemIndex * 29) % 3000);
            let reservedQuantity = 0;
            let availableQuantity = 0;
            let reorderLevel = 0;
            let stockStatus = '';
            const statusSeed = (warehouseIndex + itemIndex) % 20;
            if (statusSeed === 0) {
