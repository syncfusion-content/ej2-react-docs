...
Customize the React Query Builder UI appearance by overriding its default CSS styles. The following table lists the CSS classes and their corresponding React Query Builder UI components. Additionally, you can create custom themes using the [Theme Studio](https://ej2.syncfusion.com/themestudio/?theme=material). Note that the Theme Studio defaults to the `material` theme; switch the `?theme=` query-string to `tailwind3`, `bootstrap5`, `fluent2`, or `bootstrap4` to match your project's styling.
